...
First, I have created the `Hat` class in `prob_calculator.py`. The class takes a variable number of arguments that specify the number of balls of each color that are in the hat. For example, a class object could be created in any of these ways:
```
...
A hat will always be created with at least one ball. The arguments passed into the hat object upon creation will be converted to a `contents` instance variable. `contents` is a list of strings containing one item for each ball in the hat. Each item in the list is a color name representing a single ball of that color. For example, if your hat is `{"red": 2, "blue": 1}`, `contents` will be `["red", "red", "blue"]`.

The `Hat` class has a `draw` method that accepts an argument indicating the number of balls to draw from the hat. This method removes balls at random from `contents` and return those balls as a list of strings. The balls do not go back into the hat during the draw, similar to an urn experiment without replacement. If the number of balls to draw exceeds the available quantity, return all the balls.

Next, there is an `experiment` function in `prob_calculator.py`. This function accepts the following arguments:
...
The unit tests for this project are in `test_module.py`. I have imported the tests from `test_module.py` to `main.py` for your convenience. 
